    ...
    return (
        <div data-sender={props.sender} className={`flex ${activeUser.name !== props.sender ? "justify-start" : "justify-end"} mb-4`}>
            ...
            />
                        ...
                        {activeUser.name === props.sender && <div className={`ml-2 py-3 px-4 ${activeUser.name === props.sender ? "bg-gray-400" : "bg-blue-400"} rounded-br-3xl rounded-tr-3xl rounded-tl-xl rounded-bl-xl text-white`}>
                {props.text}
            </div>}
            {activeUser.name !== props.sender && <div className={`ml-2 py-3 px-4 ${activeUser.name === props.sender ? "bg-gray-400" : "bg-blue-400"} rounded-br-3xl rounded-tr-3xl rounded-tl-xl rounded-bl-xl  text-white`}>
                {props.text}
